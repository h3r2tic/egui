    ...
    pub fn on_event(
        ...
        match event {
            ...
            WindowEvent::ModifiersChanged(state) => {
                self.egui_input.modifiers.alt = state.state().alt_key();
                self.egui_input.modifiers.ctrl = state.state().control_key();
                self.egui_input.modifiers.shift = state.state().shift_key();
                self.egui_input.modifiers.mac_cmd =
                    cfg!(target_os = "macos") && state.state().super_key();
                self.egui_input.modifiers.command = if cfg!(target_os = "macos") {
                    state.state().super_key()
                } else {
    ...
    pub fn handle_platform_output(
        ...
        if let Some(egui::Pos2 { x, y }) = text_cursor_pos {
            window.set_ime_cursor_area(
                ...
                winit::dpi::LogicalSize::new(10, 10),
            );
        }
